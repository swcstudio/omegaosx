...
## Overview
The comps/ directory in OmegaOS (rebranded from Asterinas) provides modular kernel components for hardware drivers and subsystems, enabling hot-plug and extensible support for devices like virtio, pci, network, block, input, framebuffer, and time. Total: 153 files, ~10k LOC. Focus: Component-based architecture using comp-sys for init/register, tying to WEB3.ARL via secure modular AR peripherals (e.g., virtio for guest isolation, pci for DMA caps).
