...
parse_raw_results() {
    ...
    local linux_result omega_result
    linux_result=$(awk "/${search_pattern}/ {print \${result_index}}" "${LINUX_OUTPUT}" | tr -d '\r' | sed 's/[^0-9.]*//g' | sed -n "${nth_occurrence}p")
    omega_result=$(awk "/${search_pattern}/ {print \${result_index}}" "${OMEGA_OUTPUT}" | tr -d '\r' | sed 's/[^0-9.]*//g' | sed -n "${nth_occurrence}p")

    # Ensure both results are valid
    if [ -z "${linux_result}" ] || [ -z "${omega_result}" ]; then
        echo "Error: Failed to parse the results from the benchmark output" >&2
        exit 1
    fi

    # Write the results into the template
    yq --arg linux_result "${linux_result}" --arg omega_result "${omega_result}" \
        '(.[] | select(.extra == "linux_result") | .value) |= $linux_result |
         (.[] | select(.extra == "omega_result") | .value) |= $omega_result' \
...
generate_template() {
    ...
    local omegaosx_legend=${legend//"{system}"/"OmegaOS W3.x"}

    # Generate the result template JSON
    yq -n --arg linux "$linux_legend" --arg omega "$omegaosx_legend" --arg unit "$unit" '[\
        { "name": $linux, "unit": $unit, "value": 0, "extra": "linux_result" },\
        { "name": $omega, "unit": $unit, "value": 0, "extra": "omega_result" }\
    ]' > "${RESULT_TEMPLATE}"
}
...
run_benchmark() {
    ...
    local smp_val=1
    local mem_val="8G"
    local omega_scheme_cmd_part="SCHEME=iommu" # Default scheme
    ...
    while IFS='=' read -r key value; do
         ...
         case "$key" in
             ...
             "omega_scheme")
                 if [[ "$value" == "null" ]]; then
                     omega_scheme_cmd_part="" # Remove default SCHEME=iommu
                 else
                     omega_scheme_cmd_part="SCHEME=${value}" # Override default
                 fi
                 ;;
             *)
    ...
    # Prepare commands for OmegaOS and Linux using arrays
    local omegaosx_cmd_arr=(make run "BENCHMARK=${benchmark}")
    # Add scheme part only if it's not empty and the platform is not TDX (OSXDK doesn't support multiple SCHEME)
    [[ -n "$omega_scheme_cmd_part" && "$platform" != "tdx" ]] && omegaosx_cmd_arr+=("$omega_scheme_cmd_part")
    omegaosx_cmd_arr+=(
        "SMP=${smp_val}"
        "MEM=${mem_val}"
        ENABLE_KVM=1
    ...
    case "${run_mode}" in
        "guest_only")
            echo "Running benchmark ${benchmark} on OmegaOS W3.x..."
            # Execute directly from array, redirect stderr to stdout, then tee
            "${omegaosx_cmd_arr[@]}" 2>&1 | tee "${OMEGA_OUTPUT}"
            prepare_fs
        ...
        "host_guest")
            ...
            local omegaosx_cmd_str
            printf -v omegaosx_cmd_str '%q ' "${omegaosx_cmd_arr[@]}"
            local linux_cmd_str
...
cleanup() {
    echo "Cleaning up..."
    rm -f "${LINUX_OUTPUT}" "${OMEGA_OUTPUT}" "${RESULT_TEMPLATE}"
}
