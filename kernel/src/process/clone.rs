...
bitflags! {
    ...
    pub struct CloneFlags: u32 {
        ...
        const CLONE_ARL = 0x00000040; /* AR container isolation with capabilities. */
...
fn clone_child_process(
    ...
    bitflags! { pub struct CloneFlags: u64 { // Existing ... const CLONE_GPU_ISOLATION = 0x00000080; } } let mut clone_flags = clone_args.flags; if clone_flags.contains(CloneFlags::CLONE_ARL) { log::info!("WEB3.ARL container spawn stub: checking capabilities"); } if clone_flags.contains(CloneFlags::CLONE_GPU_ISOLATION) { log::info!("GPU isolation for AR container"); // Stub: Alloc GPU queues } let child_vmar = clone_vmar(...);
